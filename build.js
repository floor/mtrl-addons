...
const buildApp = async () => {
  try {
    ...
    console.log(`  CJS bundle: ${(await Bun.file(JS_OUTPUT).size / 1024).toFixed(2)} KB`)
    console.log(`  ESM bundle: ${(await Bun.file(MJS_OUTPUT).size / 1024).toFixed(2)} KB`)

    // Generate type definitions
    console.log('Generating TypeScript declarations...')
    const tscProcess = Bun.spawn(['tsc', '--emitDeclarationOnly', '--outDir', DIST_DIR], {
      cwd: __dirname
    })
